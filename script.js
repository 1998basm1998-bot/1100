...
function updateNetworkStatus(status) {
    // تم إلغاء التنبيهات الخاصة بالشبكة
}

async function saveOperationToQueue(action, id, customerData) {
    localStorage.setItem('cachedCustomers', JSON.stringify(allCustomersData));
}
...
async function initData() {
    let cachedCustomers = JSON.parse(localStorage.getItem('cachedCustomers')) || [];
    allCustomersData = cachedCustomers;
    if (currentLoggedTowerCode) window.renderCustomers();

    let cachedTowers = JSON.parse(localStorage.getItem('cachedTowers')) || [];
    if (cachedTowers.length === 0) {
        cachedTowers = [{ towerCode: "1234", towerName: "برج تجريبي (للعرض)", ownerName: "مدير تجريبي" }];
        localStorage.setItem('cachedTowers', JSON.stringify(cachedTowers));
    }
...
window.checkCode = function() {
    ...
    allTowersData = JSON.parse(localStorage.getItem('cachedTowers')) || [];
...
window.renewSubscription = function(id) {
    window.showInputModal("أدخل مبلغ التجديد:", async (amount) => {
        ...
        if (customer) {
            ...
            let start = customer.startDate ? new Date(customer.startDate) : new Date();
